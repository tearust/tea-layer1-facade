...
runSample(null, async (layer1)=>{
  ...
  let alice_dai = await api.query.assets.daiStore(alice.address);
  console.log('alice dai =>', alice_dai.toHuman());
  let bob_dai = await api.query.assets.daiStore(bob.address);
  console.log('bob dai =>', bob_dai.toHuman());

  // transfer 100 dai from alice to bob
  console.log('transfer 10 dai from alice to bob');
  const tx = api.tx.assets.transferDai(bob.address, 10);
  await layer1.sendTx(alice, tx);

  alice_dai = await api.query.assets.daiStore(alice.address);
  console.log('alice dai =>', alice_dai.toHuman());
  bob_dai = await api.query.assets.daiStore(bob.address);
  console.log('bob dai =>', bob_dai.toHuman());

  let alice_cml = await api.query.assets.cmlStore(alice.address);
  ...
  alice_cml = await api.query.assets.cmlStore(alice.address);
  console.log('alice cml =>', alice_cml.toHuman());
  alice_dai = await api.query.assets.daiStore(alice.address);
  console.log('alice dai =>', alice_dai.toHuman());
});